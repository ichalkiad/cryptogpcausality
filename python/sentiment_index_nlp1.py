    ...
    for t in ts:
        ...
        for j in range(len(idxs)):
            ...
            try:
                ...
                trx_num = 0
                trx_sum = []
            total_sum = btc_num + eth_num + ltc_num + xrp_num + trx_num
            total_summaries.append(btc*(btc_num/total_sum) + eth*(eth_num/total_sum) + ltc*(ltc_num/total_sum)
                              + trx*(trx_num/total_sum) + xrp*(xrp_num/total_sum))
            total_meta.append([btc_sum, eth_sum, ltc_sum, xrp_sum, trx_sum])
            btc_weights.append(btc_num / total_sum)
            eth_weights.append(eth_num / total_sum)
            ltc_weights.append(ltc_num / total_sum)
            xrp_weights.append(xrp_num / total_sum)
            trx_weights.append(trx_num / total_sum)
        ...
        for i in range(len(idxs)):
            if i in gap_indices:
                ...
                idxss.append(idxs[i])
        ftxt.close()
        if not os.path.isfile(DIR_out + "/project_weights.pickle"):
            with open(DIR_out + "/project_weights.pickle", "wb") as f:
