...
function [C_XY, C_YX, chi2_XY, chi2_YX, hyperparameters] ...
                 = ...
         Lois83_GPC_fin_n_NLP4_XnotT(lag, pairs, if_returns, ...
                                       if_caus_in_mean, testing_mode, window_length)
...
name = [name_p1,'_',name_p2,'_',name_data,'_',name_cause, '_lag',num2str(lag),'_winL_',num2str(window_length),'_NLP4_XnotT']
        for t_i = 1:dates_points_nr

            dates_start  = dates_points(t_i);
            dates_end    = dates_start + window_length -1;

            data(:,1)    = zscore(data_used_NLP(dates_start:dates_end,1));
            data(:,2)    = zscore(data_used_NLP(dates_start:dates_end,2));
            data(:,3)    = zscore(data_used_NLP(dates_start:dates_end,3));
            data(:,4)    = zscore(data_used_fin(dates_start:dates_end,pair_i1));
            Data_cuts(:,:,t_i) = data;
        end
...
[C_XY, C_YX, chi2_XY, chi2_YX, hyperparameters] ...
                   = ...        
Lois82_optim_GPC_multimodal_use_X_insteadof_T...
                            (Data_cuts,1, to_loop,...
                             name, lag, where_cause);
